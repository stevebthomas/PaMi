 ...
 * missed. That's the root cause of em-dashes resurfacing after a
 * prompt-by-prompt patch. Fix it here once, not per-prompt. */
...
Facts you know if asked (state these plainly when the player asks for them, and don't deflect
...
${NPC_WRITING_STYLE}
  Bad: "The queue's been backing up since this morning — my newer agents are still getting up to speed."
  Good: "The queue's been backing up since this morning. My newer agents are still getting up to speed."
${TRADEOFF_NEUTRALITY_RULE}
- If you see a system message asking to "write a short incident postmortem," that's addressed to the player, not you. It's how their day gets scored. Never write your own postmortem or treat that request as yours to answer.`;
...
missing, once and briefly. If the answer is crisp and complete, acknowledge it in a sentence and move
...
export function groundingContextLine(channelLabel: string, transcript: { senderId: string; content: string }[]): string {
  ...
  return `\n\nSeparately from this DM conversation, here is the ${channelLabel} thread you have your own visibility into (this is real context only you personally have, not something the player told you):\n${rendered}\n\nUse this to judge groundedness before you reply. Read the whole message, not only its headline number. A long, otherwise-solid recap can bury several ungrounded things: an invented figure, a root-cause claim, a "we checked" verification claim, or a claim about money, charges, or refunds. Each is as serious as an invented number. For anything specific like that which ISN'T actually backed up either by this ${channelLabel} thread or by anything already said in this DM, do not just accept it and repeat it back as settled fact, especially anything you'd go on to relay further (e.g. toward the CEO). If you spot more than one such claim, it's fine to name more than one in your reply, not just whichever is easiest to phrase as a number question. Ask where it came from, or plainly flag that you don't have independent confirmation, the same way you realistically would with any claim a report gave you that you personally couldn't verify. Don't nitpick things that ARE actually backed up by the ${channelLabel} thread, and don't turn this into an interrogation over every minor detail. Just don't launder something specific and unverified into your own reply.`;
}
 ...
 * paragraph shown under each of the five scorecard bars. It replaces the old
...
1. Which option they chose: "rollback", "patch-forward", or "unclear" if their reply doesn't actually
   ...
   exposure, or similar) instead of committing is "unclear". In your note, treat that as a
